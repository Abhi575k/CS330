...
//  condition variable global locks
struct sleeplock s_lock;
struct cond_t c_lock;
...
void condsleep(struct cond_t *chan, struct sleeplock *lk)
{
  struct proc *p = myproc();
  uint xticks;
...
int wakeupone(void *chan)
{
  ...
  for (p = proc; (p < &proc[NPROC] && ok == 1); p++)
  {
    if (p != myproc())
    {
      acquire(&p->lock);
...
// // semsleep
// void semsleep(struct semaphore *chan, struct sleeplock *lk)
// {
//   struct proc *p = myproc();
//   uint xticks;
...
void buffer_sem_init()
{
  ...
  for (int i = 0; i < SIZE; i++)
  {
    ...
    // sem_post(&(bounded_buffer[i].deleted));
  }
  bb_head = 0;
  bb_tail = 0;
  // printf("buffer_sem_init\n"); // debugging
  return;
}
